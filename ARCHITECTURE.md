...
│   │   │   ├── live_import_commands.rs# HCI 动态加载（WPS/FTS 注入，32 位直连 DLL）
│   │   │   ├── search_commands.rs    # 日志拉取(get_logs)/清空(clear_logs)
...
用户改值 → 记录 changes → invoke("apply_config_changes")
  → config::editor 写 INFO → 返回修改后的 INFO 字节存入前端状态
  （注：磁盘落盘 save_dcf 未接线，当前仅在内存中修改）
```

**HCI 动态加载流程（32 位直连 LiveImportAPI.dll；64 位 dev 构建返回诊断、不可用）：**
```
...
1. **烧录功能**：已移除。前端仅保留一个模拟按钮（setTimeout 假进度）；后端烧录引擎 `core/flash` 与 `start_flash`/`cancel_flash`/`get_flash_progress` 命令已删除。
2. **32 位约束**：`lc3.dll` 等为 32 位，整个后端必须 32 位构建。
3. **MSI 图标**：缺 `.ico`，仅能产出 exe。
4. **日志搜索**：仅前端 WASM（nucleo）实现，后端 `core/search` 已移除。
